...
export default function ClientePago() {
  ...
  // - tabla `tickets`: captura pagos manuales desde el panel operario
  // - tabla `pagos`: captura el pago registrado por el webhook de Mercado Pago
  // Ambos canales disparan la animación de confirmación.
  useEffect(() => {
    if (!ticket) return;

    // Canal 1: cambio de estado en el ticket
    const ticketChannel = supabase
      .channel(`ticket-status-${ticket.id_ticket}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
    ...
    // Canal 2: inserción de un nuevo registro en pagos para este ticket
    // El webhook de MP inserta aquí justo después de aprobar el pago.
    const pagosChannel = supabase
      .channel(`pagos-ticket-${ticket.id_ticket}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (_payload) => {
          // El pago quedó guardado en la base de datos → mostrar confirmación
          setPagado(true);
        }
      )
      .subscribe();
  ...
  const buscarTicket = async () => {
    ...
    try {
      ...
      const diffHrs = Math.ceil(diffMs / (10 * 60 * 60));
      const total = diffHrs * 10;
  ...
  // Confirmación manual (para pruebas o pagos en efectivo desde el panel)
  const confirmarPago = async () => {
    ...
    try {
      ...
      // La suscripción a `pagos` detectará este INSERT y activará setPagado(true)
      // pero también lo hacemos aquí como respaldo inmediato:
              ...
              <div className="dark-card p-8 bg-white/[0.02] border-brand/10">
                ...
                <div className="flex items-center gap-4 py-4 border-y border-white/5 mb-6">
                  ...
                  <p className="text-sm font-bold text-dark-muted">
                    Tiempo transcurrido: <span className="text-white">{Math.floor((new Date() - new Date(ticket.hora_entrada)) / (1000 * 60))} min</span>
                  </p>
                </div>
        ...
        <div className="animate-in slide-in-from-right duration-700">
          <div className="dark-card p-10 bg-white/[0.02] border-white/5 relative overflow-hidden group">
            ...
            <div className="relative z-10 text-center flex flex-col items-center">
              ...
              <div className="bg-white p-6 rounded-[2rem] mb-10 shadow-2xl relative border-4 border-[#009EE3]/10">
                {!ticket && (
                  <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center rounded-[2rem] z-20">
                    <p className="px-6 text-[10px] font-black text-black/40 uppercase tracking-widest leading-relaxed text-center">
                      Primero valide su ticket<br />en el panel izquierdo
                    </p>
                  </div>
                )}
                {ticket && !initPoint && (
              ...
              {ticket && (
                <div className="w-full mt-2 flex flex-col items-center justify-center p-4 bg-[#009EE3]/10 border border-[#009EE3]/20 rounded-2xl relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent shimmer-effect"></div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 border-2 border-[#009EE3]/30 border-t-[#009EE3] rounded-full animate-spin"></div>
                    <span className="text-xs font-black text-[#009EE3] uppercase tracking-widest">Aguardando pago en la App...</span>
                  </div>
                </div>
              )}
